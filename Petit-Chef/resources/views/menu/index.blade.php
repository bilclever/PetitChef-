...
@if($dishes->isEmpty())
    <div style="text-align:center;padding:60px 20px;color:var(--mid-gray)" data-menu-page="1">
...
@else
    <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(240px,1fr));gap:18px" data-menu-page="1">
        @foreach($dishes as $dish)
        @php $isOpen = $dish->cook ? $dish->cook->isShopOpen() : false; @endphp
        <div data-cook-id="{{ $dish->cook_id }}" data-dish-id="{{ $dish->id }}"
            style="background:var(--warm-white);border:1px solid var(--border);border-radius:12px;overflow:hidden;transition:box-shadow .18s,transform .18s,opacity .3s;display:flex;flex-direction:column;height:100%;{{ $isOpen ? '' : 'opacity:.55' }}"
            ...
            <div style="height:160px;position:relative;display:flex;align-items:center;justify-content:center;background:linear-gradient(135deg,#F5DEB3,#DEB887)">
                @if($dish->photo_path)
                    <img data-dish-img src="{{ asset('storage/'.$dish->photo_path) }}" style="width:100%;height:100%;object-fit:cover" alt="{{ $dish->name }}">
                @else
                ...
                <div style="position:absolute;top:10px;right:10px;background:rgba(255,255,255,.9);border-radius:20px;padding:3px 10px;font-size:11px;font-weight:600;border:1px solid var(--border);{{ $dish->quantity <= 3 ? 'color:var(--terracotta);border-color:var(--terracotta)' : '' }}">
                    <span data-dish-stock>{{ $dish->quantity }} restants</span>
                </div>
                ...
                {{-- Badge fermé --}}
                @if(! $isOpen)
                <div data-shop-overlay style="position:absolute;inset:0;background:rgba(0,0,0,.35);display:flex;align-items:center;justify-content:center">
            ...
            <div style="padding:14px 16px;display:flex;flex-direction:column;flex:1">
                <a href="{{ route('dish.show', $dish) }}" style="text-decoration:none;color:inherit">
                    <div data-dish-name style="font-family:'Fraunces',serif;font-size:16px;font-weight:500;margin-bottom:4px">{{ $dish->name }}</div>
                </a>
                ...
                <div style="margin-top:auto;padding-top:8px">
                    <div style="display:flex;align-items:center;justify-content:space-between">
                        <div data-dish-price style="font-family:'Fraunces',serif;font-size:17px;font-weight:700;color:var(--terracotta)">
                        ...
                        <div style="font-size:11px;color:var(--mid-gray);display:flex;align-items:center;gap:4px">
                            <span data-shop-dot style="width:6px;height:6px;border-radius:50%;background:{{ $isOpen ? '#2ecc71' : '#c0392b' }};display:inline-block;flex-shrink:0"></span>
                            <span data-shop-label>{{ $dish->cook->name }}</span>
                        </div>
                    </div>
                    ...
                    <form method="POST" action="{{ route('cart.add', $dish) }}" style="display:flex;gap:8px;align-items:center;margin-top:12px">
                        @csrf
                        <input data-add-input type="number" name="quantity" class="pc-input" min="1" max="{{ $dish->quantity }}" value="1"
                            style="width:78px;padding:7px 9px" @disabled(! $isOpen || $dish->quantity < 1)>
                        <button data-add-btn type="submit" class="pc-btn pc-btn-primary" style="padding:7px 11px;flex:1;justify-content:center"
